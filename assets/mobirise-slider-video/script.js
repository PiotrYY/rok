(function(){
    ...
    $(document).on('add.cards change.cards', function(event){
        ...
        if (!$(event.target).hasClass('rok-slider')) return;
        ...
        $(event.target).outerFind('[data-bg-video-slide]').each(function(){
            ...
            if (!isInit) {
                $('.rok-background-video-preview', this).remove();
            }

            var videoId = getVideoId( $(this).attr('data-bg-video-slide') );
            if (!videoId) return;

            var $preview = $('<div class="rok-background-video-preview"></div>').css({
                display: 'none',
                backgroundSize: 'cover',
                backgroundPosition: 'center'
            });

            $('.rok-overlay, .container-slide', this).before($preview);
            ...
            if (isInit && isDesktop && !isBuilder && $.fn.YTPlayer && !$(this).find('.playerBox').length) {
                ...
                $('.container-slide', this)
                    .before('<div class="rok-background-video"></div>').prev()
                    .YTPlayer($.extend({
                        videoURL: videoId,
                        containment: 'self',
                        showControls: false,
                ...
                var $overlay = $(this).find('.rok-overlay');
                if ($overlay.length) {
                    $('.YTPOverlay', this).css({
        ...
        if (isInit) {
            ...
            $(this).on('slide.bs.carousel','section.rok-slider', function(event){
            ...
            $(this).on('slid.bs.carousel','section.rok-slider', function(event){
